...
public class battleManagerScript1 : MonoBehaviour
{
    public Camera CSS;
    ...
    {
        Status = true;
        var startUnit = starter.GetComponent<UnitScript1>();
        var receiverUnit = receiver.GetComponent<UnitScript1>();
        ...
        {
            GameObject tempParticle = Instantiate(receiverUnit.GetComponent<UnitScript1>().damagedParticle, receiver.transform.position, receiver.transform.rotation);
    ...
    {
        if (unitToCheck.GetComponent<UnitScript1>().currentHealthPoints <= 0)
        {
            return true;
        }
        return false;
    ...
    public IEnumerator Attack(GameObject unit, GameObject enemy)
    {
        ...
        while (Status)
        {
            StartCoroutine(CSS.camShake(.2f, unit.GetComponent<UnitScript1>().attackDamage, GetDirection(unit, enemy)));
            if (unit.GetComponent<UnitScript1>().attackRange == enemy.GetComponent<UnitScript1>().attackRange && enemy.GetComponent<UnitScript1>().currentHealthPoints - unit.GetComponent<UnitScript1>().attackDamage > 0)
            {
                StartCoroutine(unit.GetComponent<UnitScript1>().displayDamageEnum(enemy.GetComponent<UnitScript1>().attackDamage));
                StartCoroutine(enemy.GetComponent<UnitScript1>().displayDamageEnum(unit.GetComponent<UnitScript1>().attackDamage));
            }
    ...
    public IEnumerator RePosition(GameObject unit, Vector3 endPoint)
    {
        ...
        unit.GetComponent<UnitScript1>().wait();
    }
